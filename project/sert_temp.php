...
$username=iconv('UTF-8', 'windows-1251',"Иванов Петр");
...
/*  foreach($_SESSION['modules_user_passed'] as $index=>$val){
	...
	$modulenames[]=iconv('UTF-8', 'windows-1251','модуль 1');
	$modulenames[]=iconv('UTF-8', 'windows-1251','модуль 2');
	$modulenames[]=iconv('UTF-8', 'windows-1251','модуль 3');
...
//username
	$pdf->SetFont('San_ArialBD','',20);
	  $pdf->MultiCell(0,10,"Иванов Петр",0,"C");
	   $pdf->Ln();
...
//$pdf->Output();
$path_filename = 'sertificates/temp.pdf';
$pdf->Output($path_filename,'F');

$anchor_sert = iconv('windows-1251','UTF-8','
<body bgcolor="#BCDAAE">
<br><a href="'.$path_filename.'" target="_blank">Скачать сертификат:    '.'temp.pdf'.'</a></br>
');
